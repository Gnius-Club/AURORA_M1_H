...
class AuroraIDE {
    constructor() {
        ...
        // Module data - DIFICULTAD REVERTIDA A LA VERSIÓN ANTERIOR
        this.modules = {
            energia: {
                id: "energia",
  ...
  return decision;
    ...
    markModuleComplete(moduleId) {
        ...
        if (this.completedModules.size === 6) {
            ...
            this.finalTime = document.getElementById('missionTimer').textContent; 
    ...
    showCompletionScreen() {
        ...
        document.getElementById('finalTimeDisplay').textContent = this.finalTime;
...
// *** LA CORRECCIÓN CRÍTICA Y DEFINITIVA ESTÁ AQUÍ ***
// Se asegura de que solo se cree UNA instancia de la aplicación.
let auroraInstance = null;

document.addEventListener('DOMContentLoaded', () => {
    // Crea la única instancia y asígnala a la variable global y a la local.
    if (!auroraInstance) {
        auroraInstance = new AuroraIDE();
        window.aurora = auroraInstance;
    }
});

// Resume audio context on user interaction (required by browsers)
document.addEventListener('click', function resumeAudio() {
    // Este listener comprueba la instancia global 'window.aurora'.
